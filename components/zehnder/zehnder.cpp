...
void ZehnderRF::control(const fan::FanCall &call) {
  ...
  /*
   * Named preset selected from Home Assistant.
   *
   * The actual Zehnder command is still generated by the
   * existing YAML on_preset_set automation.
   */
  if (call.has_preset_mode()) {
    ESP_LOGD(
        TAG,
        "Fan preset changed to: %s",
        call.get_preset_mode()
    );

    // Store the selected preset in the ESPHome Fan entity.
    this->set_preset_mode_(call.get_preset_mode());
  }

  /*
   * Preserve the existing numeric speed behaviour.
   */
  ...
  switch (this->state_) {
    case StateIdle:
      /*
       * If a preset was selected, the YAML on_preset_set
       * automation is responsible for calling setSpeed().
       *
       * Only send a speed command for a conventional
       * speed-based FanCall.
       */
      if (call.get_speed().has_value() ||
